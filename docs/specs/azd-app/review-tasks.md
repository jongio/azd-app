...
All review tasks complete. See [docs/archive/azd-app-archive-003.md](../../archive/azd-app-archive-003.md) for history.

## Summary of Findings

**CRITICAL fixes applied:**
- CLI Azure-not-configured error messaging updated to reference current `logs.analytics` config
- Mode switching shortcut now uses `/api/mode` API to keep backend + UI in sync
- Unified LogsView now supports Azure preset time ranges and polling behavior

**HIGH priority fixes completed:**
- ✅ Go Azure logs standalone: Service name propagation through Log Analytics queries (now maps Azure resource names back to azure.yaml service names)
- ✅ Go Azure logs standalone: Error bubbling when all resource queries fail (provides actionable error messages with detailed failure info)
- ✅ Go Azure logs standalone streaming: stderr pollution (gated behind AZD_APP_DEBUG flag)
- ✅ Go Azure logs standalone streaming: 24h window alignment (respects user-requested --since parameter)

**Test coverage:** 24/24 dashboard vitest passed, all Go unit tests passed, 3 minor test gaps identified (low priority).

See archived tasks for full details and recommended fix owners.

## DONE  
See [docs/archive/azd-app-archive-003.md](../../archive/azd-app-archive-003.md) for completed review tasks.

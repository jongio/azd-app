...
## Health Diagnostic Tooltips

### Overview

When viewing services in the dashboard, you can see detailed diagnostic information about unhealthy services by hovering over the health status icon. This provides quick insight into what's failing and how to fix it, without leaving the dashboard view.

### Using Diagnostic Tooltips

**Accessing Tooltips:**
1. Open the dashboard with `azd app run`
2. Look for the dual-status badge on each service card
3. Hover over the **health icon** (right circle in the badge)
4. A detailed diagnostic tooltip appears after 400ms

**What's in the Tooltip:**

The tooltip shows comprehensive health diagnostic information:

- **Health Status**: Current status (Healthy, Degraded, Unhealthy, Unknown)
- **Check Details**: Type of health check performed (HTTP, TCP, Process)
- **Endpoint/Target**: What was checked (URL, port, or process)
- **Response Information**: Status code, response time, uptime
- **Error Details**: Primary error message and extended details if available
- **Consecutive Failures**: How many times the check has failed in a row
- **Suggested Actions**: Recommended next steps with relevant commands
...
Note: Service is running but health status cannot
be determined. Consider adding a health check endpoint.
...
Each diagnostic tooltip includes a **Copy Diagnostics** button that copies a formatted markdown report to your clipboard. This is useful for:

- Sharing diagnostic information with team members
- Creating bug reports
- Documenting issues
- Pasting into chat/support channels
...
When you see an unhealthy service in the dashboard:

1. **Hover on health icon** - Get immediate diagnostic details
2. **Review error message** - Understand what check failed
3. **Check suggested actions** - See recommended next steps
4. **Copy diagnostics** - Click to copy full report for sharing
5. **View logs** - Click "View Logs" button or use suggested command
6. **Apply fixes** - Follow suggested actions to resolve
7. **Monitor recovery** - Health icon updates automatically when fixed

### Dashboard Integration

The health diagnostic tooltips are integrated throughout the dashboard:

| View | Tooltip Location | What It Shows |
|------|------------------|---------------|
| Service Cards | Health icon (dual-status badge) | Full diagnostic details |
| Service Table | Health status column | Same diagnostic information |
| Console View | Pane header health badge | Service-specific diagnostics |

### Accessibility

The diagnostic tooltips are designed with accessibility in mind:

- **Keyboard Navigation**: Press `Tab` to focus on health icons, `Enter` to open tooltip
- **Screen Reader Support**: ARIA labels describe health status and actions
- **Reduced Motion**: Respects `prefers-reduced-motion` setting
- **High Contrast**: Maintains readability in high contrast modes

### Configuration

Health diagnostic information is automatically populated from health check results. No additional configuration is needed, but you can enhance diagnostics by:

**Implementing Rich Health Endpoints:**
```python
...
The `details.error` field becomes the **Error Details** in the tooltip.

## Related Documentation

- [Service States and Health](../features/service-states.md) - Understanding lifecycle states vs health status
- [Health Check Troubleshooting](../troubleshooting/health-checks.md) - Common health check errors and solutions

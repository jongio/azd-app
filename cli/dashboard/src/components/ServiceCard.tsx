...
export function ServiceCard({ service }: ServiceCardProps) {
  ...
  const isHealthy = (service.status === 'ready' || service.status === 'running') && service.health === 'healthy'
    ...
    <div className="group glass rounded-2xl p-6 transition-all-smooth hover:scale-[1.02] hover:border-primary/50 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <Badge 
            variant={getStatusColor(service.status, service.health)}
        ...
        <div className="flex items-center justify-between py-3 px-4 rounded-xl bg-linear-to-r from-primary/5 to-accent/5 border border-white/5 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono font-semibold text-sm text-primary">{service.port}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-medium ${
              service.health === 'healthy' ? 'text-success' : 'text-destructive'
            }`}>
              {service.health}
            </span>
          </div>
        ...
        <div className="pt-4 border-t border-white/5 space-y-1.5 text-xs text-muted-foreground">
          <div className="flex items-center justify-between">
            <span>Started</span>
            <span className="font-medium">{formatTime(service.startTime)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span>Last checked</span>
            <span className="font-medium">{formatTime(service.lastChecked)}</span>
          </div>
        </div>

...
func TestResolveBrowserTarget(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "no --web flag means no browser",
			webFlag:        false,
			expectedTarget: browser.TargetNone,
		},
		{
			name:           "--web flag opens system browser",
			webFlag:        true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			target := resolveBrowserTarget(nil)

			if target != tt.expectedTarget {
				t.Errorf("resolveBrowserTarget() = %v, want %v", target, tt.expectedTarget)
			}
		})
	}
}

func TestWebFlagBehavior(t *testing.T) {
	// This test verifies that browser only opens when --web is specified
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if target != tt.expectedBase {
				t.Errorf("resolveBrowserTarget() = %v, want %v", target, tt.expectedBase)
			}
		})
	}
}
